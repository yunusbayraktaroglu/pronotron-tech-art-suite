...
import { createBasePointer, createHoldablePointer, HoldEventDetail, ReleaseEventDetail, TapEventDetail } from "@pronotron/pointer";
...
export function PronotronPointerProvider({ children }: { children: React.ReactNode })
{
	...
	useEffect( () => {
		...
		const holdHandler = ( event: CustomEvent<HoldEventDetail> ) => {
			event.detail.holdTarget.dataset.holded = "1";
			document.documentElement.classList.add( "holding" );
			console.log( "HOLD", event );
		};
		const holdendHandler = ( event: CustomEvent<ReleaseEventDetail> ) => {
			event.detail.holdTarget.dataset.holded = "0";
			document.documentElement.classList.remove( "holding" );
			console.log( "HOLD-END", event );
		};
		const tapHandler = ( event: CustomEvent<TapEventDetail> ) => {
			//console.log( "TAP", event.detail.tapTarget, event.detail.position );
		};
